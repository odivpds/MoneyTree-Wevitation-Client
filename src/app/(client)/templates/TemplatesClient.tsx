...
export default function TemplatesClient({ initialTemplates, initialCategories }: TemplatesClientProps) {
  ...
  const groupedTemplates = useMemo(() => {
    const filtered = initialTemplates.filter(tpl =>
    ...
    if (initialCategories.length > 0) {
      categoryGroups = initialCategories.map(cat => ({
        id: cat.id,
        name: cat.name,
        slug: cat.slug,
  ...
  }, [searchQuery, initialTemplates, initialCategories]);
