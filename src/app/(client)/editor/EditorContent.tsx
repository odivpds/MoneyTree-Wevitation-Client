...
function EditorContentInner({ templateId, initialTemplate }: EditorContentProps) {
  ...
  // UX-C — Loading state is minimal now since data is pre-fetched
  useEffect(() => {
  ...
  const currentTemplateConfig = initialTemplate;
  const isHtmlTemplate = currentTemplateConfig?.type === 'html' || currentTemplateConfig?.type === 'html-js';
              ...
              if (isHtmlTemplate) {
                return (
                  <div style={{ width: '100%', height: '100%' }}>
                    <HtmlAdapter ref={iframeRef} templateId={template} data={formData as any} photo={photo} timeLeft={timeLeft} domOverrides={domOverrides} isEditable={true} />
                  </div>
                );
              }
