...
export default async function CategoryTemplatesPage({ params }: { params: Promise<{ categorySlug: string }> }) {
    ...
    <div className="space-y-8">
      <div>
        <Link
          ...
          className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white border border-[#E6DFD1] text-gray-600 hover:text-[#677359] hover:bg-[#faf7f2] transition-all duration-300 shadow-sm hover:shadow-md mb-6 group"
        >
          <ArrowLeft size={18} className="group-hover:-translate-x-1 transition-transform duration-300" />
          <span className="text-sm font-semibold tracking-wide">Kembali ke Templates</span>
        </Link>
        <div className="flex justify-between items-end">
          <div>
            <h2 className="text-4xl font-serif tracking-tight text-[#222] mb-2">
              {isUncategorized ? "Belum Berkategori" : (category?.name || "Kategori Tidak Ditemukan")}
            </h2>
            <p className="text-gray-500 text-sm">Kelola template desain undangan untuk kategori ini.</p>
          </div>
          <Link
            href={`/admin/templates/${categorySlug}/create`}
            className="cursor-pointer flex items-center space-x-2 bg-[#677359] hover:bg-[#58634c] text-white px-6 py-3 rounded-full text-sm font-medium transition-all shadow-sm"
          >
            ...
            {templates.length === 0 ? (
              ...
              templates.map((tpl) => (
                <tr key={tpl.id} className="hover:bg-[#faf7f2] transition-colors">
                  ...
                  <td className="p-5">
                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border ${tpl.type === 'react'
                  ...
                  <td className="p-5 text-right">
                    <TemplateActions id={tpl.id} slug={tpl.slug} categorySlug={categorySlug} title={tpl.name} />
                  </td>
                </tr>
              ))
            )}
